...
import {
  CONTENTFLY_SYNC_START,
  CONTENTFLY_SYNC_SUCCESS,
  ENTITIES_TO_EXCLUDE,
  SYNC_CHUNK_SIZE,
  TIMEOUT_SYNC_MINUTES,
} from "../constants";
...
export class Service {
  ...
  private syncFiles(): Promise<boolean> {
      ...
      for (let entityName in this.schema.data) {
        ...
        for (let propertyName in entityConfig.properties) {
          ...
          switch (propertyConfig.type) {
            case "file":
              subqueries.push(
                "SELECT DISTINCT " + propertyConfig.dbfield + " FROM " + dbName
              );
              break;
            case "multifile":
              let field = propertyConfig.mappedBy
                ? propertyConfig.mappedBy + "_id"
                : "file_id";
              subqueries.push(
                "SELECT DISTINCT " + field + " FROM " + propertyConfig.foreign
              );
              break;
          }
        }
    ...
    return this.store.query(statement, []).then((files) => {
      ...
      for (let index = 0; index < files.length; index++) {
        ...
        let size =
          file["type"] && file["type"].substr(0, 5) == "image"
            ? this.imageDownloadSize
            : null;
        ...
        let p = this.api
          ...
          .then(() => {
            ...
            let progress = Math.round(
              (this.currentDataCount / this.dataCount) * 100
            );
   ...
   * @returns {Promise<any>}
   */
  private syncMultijoinFromEntity(
    key: string,
    entityName: string,
    ...
    sourceJoinField =
      sourceJoinField.substr(0, 3) == "pim"
        ? sourceJoinField.substr(3)
        : sourceJoinField;
    ...
    return this.api
        ...
        if (data.length > 0) {
          ...
          var promise = new Promise((resolve, reject) => {
            this.store.importMultijoin(sourceTableName, data).subscribe(
              ...
              (error) => {
                this.logger.error(
                  "[sync.service->syncFromEntity] store.import",
                  error
                );
                resolve();
              },
      ...
      .catch((error) => {
        this.logger.error(
          "[sync.service->syncMultijoinFromEntity] api/query",
          error
        );
        return Promise.resolve([]);
      });
  ...
  private syncFromEntity(
    ...
    if (entityConfig["settings"]["type"] == "tree") {
      let joinTableName = entityConfig["settings"]["i18n"]
        ? "pim_i18n_tree"
        : "pim_tree";
    ...
    return this.api
      ...
      .then((request) => {
        ...
        if (data.length > 0) {
          ...
          var promise = new Promise((resolve, reject) => {
            ...
            this.store.import(entityName, data, importAll).subscribe(
              ...
              () => {
                ...
                let newStartFromChunK = startFromChunk + this.syncChunkSize;
                this.logger.info(
                  "[sync.service->NEW CHUNK] " + entityName,
                  newStartFromChunK
                );
          ...
          this.syncState.setLastSyncDate(
            entityName,
            this.syncState.getStartSyncDate(entityName)
          );
          this.syncState.save();
      ...
      .catch((error) => {
        this.logger.error(
          "[sync.service->syncFromEntity] api/query/" +
            entityName +
            "::" +
    ...
    this.syncState.load().then(() => {
      ...
      .catch((error) => {
        this.data.next(
          new Message(
            Mode.FROM,
            "Synchronisation wurde mit Fehlern abgebrochen.",
            0,
            0,
            0
          )
  ...
  private startSyncFrom() {
    ...
    this.data.next(
      new Message(Mode.FROM, "Prüfe Änderungen auf dem Server...")
    );
    ...
    return startPromise
      .then((deletedObjectsFromPromise) => {
        ...
        if (
          ...
          this.data.next(
            new Message(Mode.FROM, "Datenbank bereinigen...", 0, 0, 0)
          );
      ...
      .then(() => {
        ...
        this.data.next(
          new Message(
            Mode.FROM,
            "Prüfe Änderungen auf dem Server...",
            Math.round((entitiesSynced / entitiesToSync) * 100),
            entitiesSynced,
            entitiesToSync,
            "geprüft"
          )
        ...
        for (let entityName in this.schema.data) {
          ...
          let p = this.api.post("count", params).then((countRequest) => {
            countRequestAll["data"]["dataCount"] +=
        ...
        let p = this.api.post("count", params).then((countRequest) => {
          countRequestAll["data"]["filesCount"] +=
            countRequest["data"]["filesCount"];
          countRequestAll["data"]["filesSize"] +=
            countRequest["data"]["filesSize"];
          countRequestAll["hash"] = countRequest["hash"];
          entitiesSynced++;
      ...
      .then((data) => {
            ...
            switch (type) {
              case "multifile":
                var joinedTableName = propertyConfig.foreign
                  ? propertyConfig.foreign
                  : entityConfig.settings.dbname + "_" + property;
              ...
              case "multijoin":
                ...
                entities.push({
                  ...
                  srcJoinField:
                    entityConfig.settings.dbname.replace("_", "") + "_id",
                  destTableName: entityConfig.settings.dbname,
                  isMultijoin: true,
                });
        ...
        for (let index in entities) {
          ...
          if (!isMultijoin) {
            ...
            allPromises.push(
              this.syncFromEntity(entityName, lastSyncDate, startFromChunK)
            );
          } 
        }
      ...
      .then((filesAreSynced) => {
        ...
        if (filesAreSynced) {
          ...
          this.data.next(
            new Message(
              Mode.FROM,
              "Keine Änderungen auf dem Server vorhanden.",
              0,
              0,
              0
            )
    ...
    return this.getQueue()
      .then((objects) => {
        ...
        for (let object of objects) {
          if (lastObject && lastObject.entity_id != object.entity_id) {
            if (
              lastObject.mode != QueueType.deleted ||
              (lastObject.mode == QueueType.deleted && !lastIsInserted)
            ) {
        ...
        for (let deletedObjectId of deletedObjects) {
          sqlDeleteStatements.push([
            "DELETE FROM queue WHERE entity_id = ?",
            [deletedObjectId],
          ]);

          for (
            var cleanedObjectIndex = 0;
            cleanedObjectIndex < cleanedObjects.length;
            cleanedObjectIndex++
          ) {
        ...
        this.logger.info(
          "[service.startSyncTo] objects cleaned",
          cleanedObjects.length + " / " + deletedObjects.length
        );
          ...
          if (joins && joins.length > 0) {
            for (let join of joins) {
              if (
                objects.find(
                  (x) =>
                    x.entity_id == join.entity_id &&
                    x.mode == QueueType.inserted &&
                    x.id != cleanedObject.id
                )
              ) {
                joinFound = true;
        ...
        this.logger.info(
          "[service.startSyncTo] objectsToSync",
          objectsToSync.length
        );
        this.uploader.init(
          allJoinedObjects + objectsToSync.length,
          joinedObjectsToSync
        );
      ...
      .then((objects) => {
        if (
          !objects ||
          !objects.length ||
          this.currentRetryTo >= this.api.retry
        ) {
